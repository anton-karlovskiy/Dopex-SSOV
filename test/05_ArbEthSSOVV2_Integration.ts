...
import { erc20Abi, MAX_VALUE, wethHolders } from '../helper/data';
...
describe('Arbitrum ETH SSOV V2 Integration tests', async () => {
  ...
  beforeEach(async () => {
    ...
    const rewardsDistribution = await MockRewardsDistribution.deploy();

    const feeStrategy = await (
      await ethers.getContractFactory('FeeStrategy')
    ).deploy();

    ssov = await deployEthSSOV(
  ...
  it('Tests from epoch 1 through 3', async () => {
    ...
    */
    ...
    const results = await mockChainlinkAggregator.latestRoundData();
    const usdPrice = results[1];
    ...
    expect(
      ...
      BigNumber.from('995400140000000000000').toString().substring(0, 4)
    );
    ...
    expect(
      ...
      BigNumber.from('98490140000000000000').toString().substring(0, 4)
    );
    ...
    const feesChargedToUser1Epoch3 = await ssov.calculatePurchaseFees(
      usdPrice,
      newSetStrikes2Epoch3,
