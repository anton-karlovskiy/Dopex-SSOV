                  ...
                  ░▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓       ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓                  
                    ░▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓   ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓                    
                      ░▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓                      
                        ...
                        BNB SINGLE STAKING OPTION VAULT
            Mints covered calls while farming yield on single sided BNB staking farm                                                           
*/
...
contract BnbSSOV is ContractWhitelist, ReentrancyGuard, Pausable, IERC20SSOV {
    ...
    /// @notice Epoch vbnb balance per strike after accounting for rewards
    /// @dev mapping (epoch => (strike => balance))
    mapping(uint256 => mapping(uint256 => uint256))
        public totalEpochStrikeVbnbBalance;
    ...
    event EmergencyWithdraw(address sender, uint256 vbnbWithdrawn);
    ...
    function emergencyWithdraw()
        ...
        returns (bool)
    {
        IERC20 vbnb = IERC20(getAddress('VBNB'));

        uint256 vbnbBalance = vbnb.balanceOf(address(this));

        vbnb.safeTransfer(msg.sender, vbnbBalance);

        emit EmergencyWithdraw(msg.sender, vbnbBalance);
    ...
    /// @dev Updates the final epoch vbnb balances per strike of the vault
    ...
    function _updateFinalEpochBalances(bool accountPremiums) internal {
        ...
        for (uint256 i = 0; i < strikes.length; i++) {
            ...
            // Update final vbnb balances for epoch and strike
            totalEpochStrikeVbnbBalance[currentEpoch][strikes[i]] =
                totalEpochStrikeDeposits[currentEpoch][strikes[i]] -
                settlement;

            if (accountPremiums) {
                totalEpochStrikeVbnbBalance[currentEpoch][
                    strikes[i]
    ...
    function bootstrap() external onlyOwner whenNotPaused returns (bool) {
        ...
        for (uint256 i = 0; i < epochStrikes[nextEpoch].length; i++) {
            ...
            string memory name = concatenate('BNB-CALL', strike.toString());
            ...
            _erc20.mint(
                address(this),
                vbnbToBnb(totalEpochStrikeDeposits[nextEpoch][strike])
            );
        }
    ...
    /**
     * @notice Deposits VBNB into the ssov to mint options in the next epoch for selected strikes
    ...
    function deposit(
        ...
        IERC20(getAddress('VBNB')).safeTransferFrom(
            msg.sender,
            address(this),
            amount
        );
    ...
    /**
     * @notice Deposit VBNB multiple times into different strike
     * @param strikeIndices Indices of strikes to deposit into
     * @param amounts Amount of VBNB to deposit into each strike index
     * @param user Address of the user to deposit for
    ...
    /**
     * @notice Internal function to handle VBNB deposits
     * @param strikeIndex Index of strike
     * @param amount Amout of VBNB to deposit
        ...
        returns (uint256, uint256)
    {
        ...
        // Get total premium for all calls being purchased in vbnb
        uint256 premium = calculatePremium(strike, amount);

        // Total fee charged
        uint256 totalFee = calculatePurchaseFees(getUsdPrice(), strike, amount);
        ...
        vbnb.safeTransferFrom(msg.sender, address(this), finalTotal);

        if (totalFee > 0) {
            // Transfer fee to FeeDistributor
            vbnb.safeTransfer(getAddress('FeeDistributor'), totalFee);
        }
    ...
    /**
     * @notice Settle calculates the PnL for the user with the settlement price and withdraws the PnL in VBNB to the user. Will also the burn the option tokens from the user.
     * @param strikeIndex Strike index for current epoch
        ...
        returns (uint256 pnl)
    {
        ...
        vbnb.safeTransfer(msg.sender, pnl - totalFee);
        ...
        returns (uint256[1] memory)
    {
        ...
        // Calculate amount of VBNB to transfer to user
        uint256 userVbnbAmount = (totalEpochStrikeVbnbBalance[withdrawEpoch][
            strike
        ] * userStrikeDeposits) /
        ...
        // Transfer VBNB to user
        IERC20(getAddress('VBNB')).safeTransfer(msg.sender, userVbnbAmount);
    ...
    /// @dev Converts vBNB to BNB
    /// @param vbnbAmount vBNB amount in 1e8
    /// @return the BNB amount in 1e18
    function vbnbToBnb(uint256 vbnbAmount) public view returns (uint256) {
        return
            (vbnbAmount *
                IVBNB(getAddress('VBNB')).exchangeRateStored() *
                1e10) / 1e28;
    }

    /// @dev Converts BNB to vBNB
    /// @param bnbAmount BNB amount in 1e18
    /// @return the vBNB amount in 1e8
    function bnbToVbnb(uint256 bnbAmount) public view returns (uint256) {
        return
            ((bnbAmount * 1e28) /
                IVBNB(getAddress('VBNB')).exchangeRateStored()) / 1e10;
    }

    /// @notice Calculate Pnl
    /// @param price price of BNB
    /// @param strike strike price of the the BNB option
    ...
    function calculatePnl(
        ...
        return
            price > strike
                ? bnbToVbnb((((price - strike) * amount) / price))
                : 0;
    }
    ...
    /**
     * @notice Returns the price of BNB/USD via chainlink
     */
