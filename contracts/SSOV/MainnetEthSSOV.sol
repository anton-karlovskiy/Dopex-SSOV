    ...
    INativeSSOV
{
    ...
    /// @dev % Slippage Tolerated when removing liquidity from curve
    uint256 public slippageTolerance = 5e8 / 10; // 0.5%
    ...
    /// @notice Premium collected per strike for an epoch currently in the contract
    /// @dev mapping (epoch => (strike => premium))
    mapping(uint256 => mapping(uint256 => uint256)) public currentEpochPremium;
    ...
    event SlippageToleranceUpdate(uint256 slippageTolerance);
    ...
    event Compound(uint256 epoch, uint256 amount);
    ...
    constructor(
        ...
        require(_steCRV != address(0), 'E1');
        require(_curve != address(0), 'E1');
        require(_yearn != address(0), 'E1');
        ...
        addresses['steCRV'] = _steCRV;
        addresses['Curve'] = _curve;
        addresses['Yearn'] = _yearn;
        ...
        // Max approve to Yearn
        IERC20(getAddress('steCRV')).safeApprove(
            getAddress('Yearn'),
            type(uint256).max
        );
    }
    ...
    /// @return Whether it was successfully updated
    function updateSlippageTolerance(uint256 _slippageTolerance)
        external
        onlyOwner
        returns (bool)
    {
        slippageTolerance = _slippageTolerance;
        emit SlippageToleranceUpdate(_slippageTolerance);
        return true;
    }
    ...
    function _updateFinalEpochBalances() internal {
        // Initialize yearn and curve
        IYearn yearn = IYearn(getAddress('Yearn'));
        ICurve curve = ICurve(getAddress('Curve'));

        // Variable to track eth claimed after withdrawing and removing liquitdity
        uint256 ethClaimed;

        if (yearn.balanceOf(address(this)) > 0) {
            // Withdraw all tokens from previous epoch
            uint256 amount = yearn.withdraw();
            // Calculating the slippage allowed
            uint256 minAmount = curve.calc_withdraw_one_coin(amount, int128(0));
            minAmount = minAmount - ((minAmount * slippageTolerance) / 1e10);
            // Remove liquidity from curve
            ethClaimed = curve.remove_liquidity_one_coin(
                amount,
                int128(0),
                minAmount
            );
        ...
        if (totalEpochDeposits[currentEpoch] > 0) {
            ...
            for (uint256 i = 0; i < strikes.length; i++) {
                uint256 totalExercisePnl = calculatePnl(
                ...
                totalEpochStrikeEthBalance[currentEpoch][strikes[i]] =
                    ...
                    currentEpochPremium[currentEpoch][strikes[i]] -
                    totalExercisePnl;
            }
        }
    }
    ...
    /**
     * @notice Internal function to handle despositing into yearn
     * @param amount Amout of ETH to deposit
     */
    function _depositYearn(uint256 amount) internal {
        // Add eth liquidity to curve
        uint256 steCRVReceived = ICurve(getAddress('Curve')).add_liquidity{
            value: amount
        }([amount, 0], 0);
        // Deposit stethCRV received to yearn
        IYearn(getAddress('Yearn')).deposit(steCRVReceived);
    }
        ...
        returns (uint256, uint256)
    {
        ...
        // Add to current epoch strike premium
        currentEpochPremium[currentEpoch][strike] += premium;
     ...
     */
    function compound() external whenNotPaused nonReentrant returns (bool) {
        require(!isEpochExpired[currentEpoch], 'E3');
        require(isVaultReady[currentEpoch], 'E20');

        // Strikes for the current epoch
        uint256[] memory strikes = epochStrikes[currentEpoch];
        // Amount of premiums in the contract
        uint256 amount;

        // Calculate total premium from all strikes in the contract
        for (uint256 i = 0; i < strikes.length; i++) {
            amount += currentEpochPremium[currentEpoch][strikes[i]];
            currentEpochPremium[currentEpoch][strikes[i]] = 0;
        }
        ...
        // Use Premiums collected to add liquidity to curve
        uint256 steCRVReceived = ICurve(getAddress('Curve')).add_liquidity{
            value: amount
        }([amount, 0], 0);
        // Deposit stCRV received into the yearn
        IYearn(getAddress('Yearn')).deposit(steCRVReceived);

        emit Compound(currentEpoch, amount);
        return true;
    }
